...
public class MBProgressHUD: UIView {
	...
	// tungxuan
	@discardableResult
	public class func show(added view: UIView, animated: Bool = true) -> MBProgressHUD {
		let hud = MBProgressHUD(withView: view)
		hud.removeFromSuperViewOnHide = true
		view.addSubview(hud)
		hud.show(animated: animated)
	...
	public class func showDone(for view: UIView, hide delay: TimeInterval, animated: Bool = true, completion: (() -> Void)? = nil) {
		DispatchQueue.main.async {
			let hud = MBProgressHUD.hudForView(view) ?? MBProgressHUD.show(added: view, animated: animated)
			let image = UIImage(systemName: "checkmark", withConfiguration: UIImage.SymbolConfiguration(pointSize: 50, weight: .bold))
			hud.mode = .customView
			let imageView = UIImageView(image: image)
			imageView.tintColor = UIColor(rgb: 0x455667).withAlphaComponent(0.7)
			hud.customView = imageView
			hud.isSquare = true
			hud.label?.text = NSLocalizedString("Done", comment: "")
			if delay > 0 {
	...
	public class func show(for view: UIView, image: UIImage, text: String, animated: Bool = true, hide delay: TimeInterval? = nil, completion: (() -> Void)? = nil) {
		DispatchQueue.main.async {
			let hud = MBProgressHUD.hudForView(view) ?? MBProgressHUD.show(added: view, animated: animated)
			hud.mode = .customView
			let imageView = UIImageView(image: image)
	...
	public class func show(for view: UIView, text: String, animated: Bool = true, hide delay: TimeInterval? = nil, completion: (() -> Void)? = nil) {
		DispatchQueue.main.async {
			let hud = MBProgressHUD.hudForView(view) ?? MBProgressHUD.show(added: view, animated: animated)
			hud.mode = .text
			hud.label?.text = text
	...
	public class func hide(for view: UIView, animated: Bool = true) {
		DispatchQueue.main.async {
			let hud = hudForView(view)
			if (hud != nil) {
				hud?.removeFromSuperViewOnHide = true
