...
/*
// MARK: UIButton Highlight
extension UIButton {
    @IBInspectable var cornerSuper: Bool {
        get {
            return self.cornerSuper
        }
        set { }
    }
    
    @IBInspectable var hlSuper: Bool {
        get {
            return self.hlSuper
        }
        set {
            if newValue {
                ...
                self.addTarget(self, action: #selector(touchDownHighlight(_:)), for: .touchDown)
                self.addTarget(self, action: #selector(touchUpHighlight(_:)), for: .touchUpInside)
                self.addTarget(self, action: #selector(touchUpHighlight(_:)), for: .touchUpOutside)
            }
            else {
                if let sv = self.superview {
                    sv.sendSubviewToBack(self)
                }
                self.removeTarget(self, action: #selector(touchDownHighlight(_:)), for: .touchDown)
                self.removeTarget(self, action: #selector(touchUpHighlight(_:)), for: .touchUpInside)
                self.removeTarget(self, action: #selector(touchUpHighlight(_:)), for: .touchUpOutside)
            }
        }
    }
    
    @IBInspectable var hlSelf: Bool {
        get {
            return self.hlSelf
        }
        set {
    ...
    @objc func touchDownHighlight(_ sender: Any) {
        if self.cornerSuper {
            if let sv = self.superview {
                self.layer.cornerRadius = sv.layer.cornerRadius
            }
        }
    ...
    @objc func touchUpHighlight(_ sender: Any) {
        if let button = sender as? UIButton {
            UIView.animate(withDuration: 0.05, delay: 0, options: [.repeat, .autoreverse], animations: {
    ...
    @IBInspectable var hlScaleSuper: Bool {
        get {
            return self.hlScaleSuper
        }
        set {
            if newValue {
                self.addTarget(self, action: #selector(touchDownHighlightScale), for: .touchDown)
                self.addTarget(self, action: #selector(touchUpHighlightScale), for: .touchUpInside)
                self.addTarget(self, action: #selector(touchUpHighlightScale), for: .touchUpOutside)
            }
            else {
                self.removeTarget(self, action: #selector(touchDownHighlightScale), for: .touchDown)
                self.removeTarget(self, action: #selector(touchUpHighlightScale), for: .touchUpInside)
                self.removeTarget(self, action: #selector(touchUpHighlightScale), for: .touchUpOutside)
            }
        }
    }
    
    @objc func touchDownHighlightScale(_ sender: Any) {
        if let superView = self.superview {
            UIView.animate(withDuration: 0.1) {
                superView.transform = .init(scaleX: 0.95, y: 0.95)
            }
        }
    }
    
    @objc func touchUpHighlightScale(_ sender: Any) {
        if let superView = self.superview {
            UIView.animate(withDuration: 0.1) {
    ...
    @IBInspectable var hlScaleSelf: Bool {
        get {
            return self.hlScaleSelf
        }
        set {
            if newValue {
                self.addTarget(self, action: #selector(touchDownHighlightScaleSelf), for: .touchDown)
                self.addTarget(self, action: #selector(touchUpHighlightScaleSelf), for: .touchUpInside)
                self.addTarget(self, action: #selector(touchUpHighlightScaleSelf), for: .touchUpOutside)
            }
            else {
                self.removeTarget(self, action: #selector(touchDownHighlightScaleSelf), for: .touchDown)
                self.removeTarget(self, action: #selector(touchUpHighlightScaleSelf), for: .touchUpInside)
                self.removeTarget(self, action: #selector(touchUpHighlightScaleSelf), for: .touchUpOutside)
            }
        }
    }
    
    @objc func touchDownHighlightScaleSelf(_ sender: Any) {
        UIView.animate(withDuration: 0.1) {
            self.transform = .init(scaleX: 0.95, y: 0.95)
        }
    }
    
    @objc func touchUpHighlightScaleSelf(_ sender: Any) {
        UIView.animate(withDuration: 0.1) {
